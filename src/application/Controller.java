...
public class Controller implements Initializable {
	...
	// Construção
	@FXML
	private Button btn_confirmacao; // Botão para confirmar compra
	@FXML
	...
	@FXML
	private Button btn_praca; // Comprar praça
	@FXML
	...
	// Array de String contendo a situação de cada terreno e o que deve aparecer
	// dentro do botão.
	...
	/*
	 * É necessário implementar a interface Iniatilizable e reescrever o método
	 * initialize. Ele ocorre assim que o programa é executado. Sua função é a
	 * criação de todos os botões para que não sejam nulos e a atribuição de cada um
	...
	/*
	 * Primeiro evento que ocorre quando o usuário inicia o jogo. É criado uma caixa
	 * de dialógo onde o usuário insere o nome da c idade. Após isso, é criado um
	 * objeto estático do tipo Cidade
	 */
	...
	/*
	 * Função para realizar duas tarefas durante o decorrer do jogo. A primeira é a
	 * de arrecadar impostos e receitas e acontece a cada 45 segundos (primeira
	 * arrecadação em 30s). A segunda é a de ocorrer eventos aleatórios a cada 20s
	...
	/*
	 * A cada mudança de páginas, os status do jogo e a situação dos terrenos são
	 * atualizados. Os textos são setados a partir dos dados do objeto cidade. É
	 * checado também se ocorrerá Game Over (Dinheiro, Felicidade ou População = 0)
	 */
	...
	/*
	 * Função que recebe duas strings (título do evento e texto do alerta) e retorna
	...
	/*
	 * Checa a situação do terreno. Caso o botão do terreno esteja com o texto
	 * "Terreno Vazio" e não esteja desabilitado é redirecionado o usuário à parte
	 * de construção. Caso não esteja vazio, é emitido o alerta perguntando ao
	 * usuário se gostaria de demolir o atual estabelecimento.
	 */
	public void checarTerreno(ActionEvent evento) {
		...
		if (botao_apertado.getText().equals("Terreno Vazio") && !botao_apertado.isDisabled()) {
			...
			if (resultado.get() == ButtonType.OK) {
				// Caso queira demolir, pegamos o número do terreno e mandamos para o método
				// demolirEstabelecimento().
				// Ex: Se for o terreno1, pegamos o número 1 e enviamos o parâmetro 0 (Terreno 1
				// - Posição 0 do array).
	...
	/*
	 * Função que mostra o que cada Estabelecimento proporciona. Por exemplo, caso o
	 * usuário clique no btn_casa, é criado um novo objeto da classe Casa e seus
	 * staus são mostrados na janela através do método mostrarDadosConstrução que
	 * manda esse objeto como param. Além disso, esse objeto torna-se a última opção
	 * escolhida pelo usuário caso ele confirme a compra (método checarCompra()).
	 */
	...
	/*
	 * Após apertar o btn_confirmar, a função vê a última opção inserida e chama os
	 * métodos de construção do estabelecimento do objeto cidade. Caso esses métodos
	 * retornem true, chama o Evento de compra realizada com sucesso e aloca o
	 * terreno com o objeto comprado (através do método alocarTerreno())
	 */
	...
	/*
	 * Método que transforma o terreno na opção comprada. Ele procura o primeiro
	 * terreno que estiver vazio e transforma-o. Além disso, adiciona o objeto
